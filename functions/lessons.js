...
export async function onRequest(context) {
  ...
  try {
    ...
    if (url.searchParams.get("debug") === "1") {
      const sample = data.results.slice(0,2).map(p => ({id:p.id, props:Object.fromEntries(Object.entries(p.properties).map(([k,v])=>[k,{type:v.type}]))}));
      return new Response(JSON.stringify(sample,null,2), {status:200,headers:cors});
    }
    ...
    data.results.forEach(page => {
      ...
      const lesson = {
        n:       num != null ? String(num).padStart(2,'0') : '',
        t:       title,
