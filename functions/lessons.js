  ...
  try {
    ...
    const res = await fetch(
      `https://api.notion.com/v1/databases/${dbId}/query`,
      {
        ...
        body: JSON.stringify({
          page_size: 200,
          sorts: [{ property: 'Lesson Number', direction: 'ascending' }],
        }),
      }
    );
    ...
    data.results.forEach(page => {
      ...
      /* Lesson number */
      const num = page.properties?.['Lesson Number']?.number
               ?? page.properties?.['Lesson number']?.number
               ?? page.properties?.['lesson_number']?.number
               ?? null;

      /* Keywords — multi-select or rich text */
      let keywords = '';
      const kwProp = page.properties?.Keywords || page.properties?.Keyword;
      if (kwProp?.multi_select) {
        keywords = kwProp.multi_select.map(k => k.name).join(' · ');
      } else if (kwProp?.rich_text) {
